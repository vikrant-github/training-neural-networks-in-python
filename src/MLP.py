...
class MultiLayerPerceptron:     
    ...
    def __init__(self, layers, bias = 1.0):
        ...
        for i in range(len(self.layers)):
            self.values.append([])
            self.network.append([])
            self.values[i] = [0.0 for j in range(self.layers[i])] # creates a list of zeros with a length equal to the number of neurons in the ith layer of a neural network.
